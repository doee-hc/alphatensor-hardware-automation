...
def find_u_v_path_num(u,v,w):
    ...
    path_num_arr = res_mul_num + res_add_num

    # 按照复杂程度排序
    path_sorted_indices = sorted(range(len(res_mul_num)), key=lambda i: res_mul_num[i] + res_add_num[i],reverse=True)
    #print("\r\nSorted indices:", sorted_indices)

    #max_path = path_sorted_indices[0]
    #path_num_max = res_mul_num[max_path] + res_add_num[max_path]

    return (path_num_arr,path_sorted_indices)

def find_w_path_num(arr):
    non_zero_counts = np.count_nonzero(arr, axis=1)
    path_sorted_indices = sorted(range(len(non_zero_counts)), key=lambda i: non_zero_counts[i],reverse=True)
    #row_with_max_non_zero = np.argmax(non_zero_counts)
    return (non_zero_counts,path_sorted_indices)
...
def print_equations(u, v, w):
    ...
    (u_v_path_num,u_v_path_sorted_indeces) = find_u_v_path_num(u,v,w)
    total_level = u_v_path_num[u_v_path_sorted_indeces[0]]
    print(f'total_level:{total_level}')

    for level_n in range(total_level):

        (u_v_path_num,u_v_path_sorted_indeces) = find_u_v_path_num(u,v,w)
        (w_path_num,w_path_sorted_indeces) = find_w_path_num(w_act)
        
        u_v_path_num_max = u_v_path_num[u_v_path_sorted_indeces[0]]
        u_v_path_num_min = u_v_path_num[u_v_path_sorted_indeces[-1]]
